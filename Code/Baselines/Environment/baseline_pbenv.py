...
class PyBulletEnvironment(gym.Env):
	...
	def __init__(self, n_steps, GUI = False, data_path = 'Trial01.npy', reward_func = 'Phi3'):
		super(PyBulletEnvironment, self).__init__()
		...
		self.action_space = spaces.Box(low=-1.2*np.pi,high=1.2*np.pi,shape=(self._actions['shape'],))
		self.observation_space = spaces.Box(low=-500,high=500,shape=(self._states['shape'],))
	...
	def step(self, action):
		...
		self.Early_Termination |= self.penaltyframes > self.penaltyframe_limit 
		...
		return (state, reward, done or self.Early_Termination, dict())

	def render(self, mode='human'):
		if self.GUI:
			print('You forgot to implement this.')
		else:
			print('You need a GUI for this.')
